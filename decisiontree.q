...
fbf:{[data;f;t;besterror;bestfeature;bestl;bestr;i]leftsplit:data[f[i]]=0;rightsplit:data[f[i]]=1;error:(inm[leftsplit[t]]+inm[rightsplit[t]])%(count data);if[error<=besterror;besterror:error;bestfeature:f[i];bestl:leftsplit;bestr:rightsplit];$[(i+1)>=(count f);(bestfeature;bestl;bestr);fbf[data;f;target;besterror;bestfeature;bestl;bestr;i+1]]};
...
kumartree:{[master;data;f;target;besterror;bestfeature;indx]
  ...
  splitdata:fbf[data;f;target;besterror;bestfeature;0;0;i:0];
  f:f[where f<>splitdata[0]];
  ...
  kumartree[master;splitdata[1];f;target;10;"none";indx+1]

  nxtr:enlist (temp,2);
  master:master,enlist(nxtr;splitdata[2]);
  kumartree[master;splitdata[2];f;target;10;"none";indx+1]
